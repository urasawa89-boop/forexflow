...
export default function Home() {
  ...
  function ImportCalc() {
    ...
    return <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.015)", borderRadius: 9, padding: 14 }}>
        ...
        {[["물품가격 (CIF)", fmtKRW(krw), null], [`관세 (${prod.tariff}%)`, "+ " + fmtKRW(tariffAmt), "#ff8c00"], ["부가세 (10%)", "+ " + fmtKRW(vat), "#ff8c00"]].map(([l, v, c], i) =>
        ...
        {SEAFOOD_DB.filter(p => p.hs !== "-").slice(0, 16).map(p =>
          <div key={p.id} style={{ background: "rgba(255,255,255,0.015)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(255,255,255,0.03)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10.5 }}>
              <code style={{ color: "#ffd666" }}>{p.hs}</code>
              <span style={{ color: p.tariff > 15 ? "#ff8c00" : "#00d278", fontWeight: 600 }}>{p.tariff}%</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8 }}>
        {oceanData.slice(0, 5).map(st =>
          <div key={st.code} style={{ background: "rgba(255,255,255,0.015)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(255,255,255,0.03)", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: Number(st.waveHeight) > 1.5 ? "#ff5050" : "#4da6ff" }}>{st.waveHeight}m</div>
  ...
  const OceanTab = () => {
    ...
    const getWaveColor = (h) => { const v = Number(h); if (v >= 2.0) return "#ff3333"; if (v >= 1.5) return "#ff8c00"; if (v >= 1.0) return "#ffd666"; return "#00d278" }
    ...
    return <div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 14 }}>
        {regions.map(r => <button key={r} style={{ ...S.bo, ...(oceanRegion === r ? { background: "rgba(0,100,255,0.1)", color: "#4da6ff" } : {}) }} onClick={() => setOceanRegion(r)}>{r}</button>)}
      </div>

      {/* Wave height summary */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
        {[["잔잔 (0~1m)", "#00d278"], ["보통 (1~1.5m)", "#ffd666"], ["약간 높음 (1.5~2m)", "#ff8c00"], ["높음 (2m+)", "#ff3333"]].map(([label, color]) => {
          const count = filtered.filter(s => { const v = Number(s.waveHeight); if (color === "#00d278") return v < 1; if (color === "#ffd666") return v >= 1 && v < 1.5; if (color === "#ff8c00") return v >= 1.5 && v < 2; return v >= 2 }).length
        ...
        {filtered.map(st => <div key={st.code} style={S.c}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.015)", borderRadius: 7, padding: "6px 8px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: "#4da6ff" }}>{st.waterTemp}<span style={{ fontSize: 10 }}>℃</span></div>
            </div>
            <div style={{ background: "rgba(255,255,255,0.015)", borderRadius: 7, padding: "6px 8px", textAlign: "center" }}>
      ...
      <div style={{ ...S.c, marginTop: 16, padding: 12, fontSize: 11, color: "rgba(255,255,255,0.3)" }}>
        💡 <strong>바다누리 API 연동 안내:</strong> 국립해양조사원(khoa.go.kr) → 바다누리 OPEN API → 무료 인증키 발급 후 <code style={{ color: "#ffd666" }}>pages/api/ocean.js</code> 파일을 추가하면 실시간 데이터로 전환됩니다. 조위, 파고, 수온, 풍향/풍속, 조류 정보 제공.
      </div>
    </div>
  }
  ...
  return <div style={S.app}>
    ...
    {alertM && <AlertMod />}
    {toast && <div style={{ position: "fixed", bottom: 20, right: 20, background: "#00d278", color: "#000", padding: "9px 18px", borderRadius: 9, fontWeight: 600, fontSize: 12.5, zIndex: 2000 }}>{toast}</div>}
  </div>
}
